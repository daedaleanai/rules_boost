...
def boost_deps():
  if "net_zlib_zlib" not in native.existing_rules():
    native.new_http_archive(
        name = "net_zlib_zlib",
        build_file = "@com_github_daedaleanai_rules_boost//:BUILD.zlib",
  ...
  if "org_bzip_bzip2" not in native.existing_rules():
    native.new_http_archive(
        name = "org_bzip_bzip2",
        build_file = "@com_github_daedaleanai_rules_boost//:BUILD.bzip2",
  ...
  if "org_lzma_lzma" not in native.existing_rules():
    native.new_http_archive(
        name = "org_lzma_lzma",
        build_file = "@com_github_daedaleanai_rules_boost//:BUILD.lzma",
  ...
  if "boost" not in native.existing_rules():
    native.new_http_archive(
      ...
      build_file = "@com_github_daedaleanai_rules_boost//:BUILD.boost",
